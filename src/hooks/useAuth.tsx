...
const isSupabaseNetworkFailure = (error: unknown): boolean => {
  const message = error instanceof Error ? error.message : String(error || '');
  return (
    /failed to fetch/i.test(message) ||
    /networkerror/i.test(message) ||
    /name_not_resolved/i.test(message) ||
    /supabase timeout/i.test(message)
  );
};
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      try {
        ...
        let sessionCheckFailedDueToNetwork = false;
        ...
        try {
          ...
          if (isSupabaseNetworkFailure(supabaseError)) {
            sessionCheckFailedDueToNetwork = true;
        ...
        if (sessionCheckFailedDueToNetwork) {
          return;
        }
