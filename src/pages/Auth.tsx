...
const formatSupabaseError = (error: unknown, language: string): string => {
  const message = error instanceof Error ? error.message : String(error || '');
  const isNetworkFailure =
    /failed to fetch/i.test(message) ||
    /name_not_resolved/i.test(message) ||
    /networkerror/i.test(message) ||
    /authretryablefetcherror/i.test(message);

  if (isNetworkFailure) {
    return language === 'ar'
      ? 'تعذر الاتصال بخدمة Supabase. تأكد من رابط المشروع في VITE_SUPABASE_URL وأنه يعمل.'
      : 'Could not connect to Supabase. Verify VITE_SUPABASE_URL and ensure the project URL is reachable.';
  }

  return message || (language === 'ar' ? 'حدث خطأ غير متوقع.' : 'Unexpected error.');
};
...
export function AuthPage() {
  ...
  useEffect(() => {
    ...
    const sessionExists = Boolean(user) || Boolean(readMockUser());
    setHasSession(sessionExists);

    if (sessionExists && !forceAuth) {
      navigate('/', { replace: true });
    }
  }, [user, navigate, forceAuth]);
        ...
        if (isLogin) {
          ...
          if (error) {
            toast({
              ...
              description: formatSupabaseError(error, language),
            });
            return;
          }
